...
public sealed class GeoValidator
{
        ...
        IProgress<ProgressReport>? progress = null)
    {
        ...
        while (reader.Read())
        {
            ...
            {
                errCount++;
                _logger.LogError("Invalid lat {lat} at place #{place}", d, id);
            }
            // rplon
            ...
            {
                errCount++;
                _logger.LogError("Invalid lon {lon} at place #{place}", d, id);
            }
            // bboxswlat
            ...
            {
                errCount++;
                _logger.LogError("Invalid bbox_sw_lat {lat} at place #{place}", d, id);
            }
            // bboxswlon
            ...
            {
                errCount++;
                _logger.LogError("Invalid bbox_sw_lon {lon} at place #{place}", d, id);
            }
            // bboxnelat
            ...
            {
                errCount++;
                _logger.LogError("Invalid bbox_ne_lat {lat} at place #{place}", d, id);
            }
            // bboxnelon
            ...
            {
                errCount++;
                _logger.LogError("Invalid bbox_ne_lon {lon} at place #{place}", d, id);
            }
        ...
        IProgress<ProgressReport>? progress)
    {
        ...
        while (reader.Read())
        {
            ...
            try
            {
                JsonConvert.DeserializeObject<GeoJson>(json);
            }
            catch (Exception ex)
            {
                _logger.LogError("Invalid GeoJSON in {table} at #{id}: " +
                    "{message}: \"{json}\"", table, id, ex.Message, json);
                errCount++;
            }
